...
for k in range(len(to_search)):
    for i in range(len(first)):
        if first[i][0] == "R":
            ...
            for i in range(int(first[i][1:])):
                if loc[x] == to_search[k][x] and loc[y] == to_search[k][y]:
                    break
                loc[y] += 1
                first_history.append([loc[x], loc[y]])
                steps += 1
    ...
    for i in range(len(second)):
        ...
        if second[i][0] == "R":
            ...
            for i in range(int(second[i][1:])):
                ...
                second_history.append([loc[x], loc[y]])
                steps += 1
    step_hist2.append(steps)
    steps = 0
    loc = [0, 0]
step_hist_sum = []
for i in range(len(step_hist)):
    step_hist_sum.append(step_hist[i] + step_hist2[i])
index = step_hist_sum.index(7677)
print(index)
...
step_hist_sum.sort()
print(step_hist_sum)
print(first_history[index])
print(second_history[index])